...
//        if songList.count == 0{
  //          restart = true
    ...
    func callAlamo(url: String){
        ...
        httpHeader = ["Authorization": "Bearer \(SPTAuth.defaultInstance()!.session.accessToken!)"]
        Alamofire.request(url, headers: httpHeader).responseJSON(completionHandler: {
            response in
            self.ParseData(JSONData: response.data!)
            print("\nIn compl handler for callAlamo\n\(SPTAuth.defaultInstance()!.session.accessToken!)\n")
        })
    }
